...
export const lexicalMarkdownHook = (
  contentName: string,
  markdownName: string,
  ...
  return {
    hooks: {
      beforeChange: [
        async ({ collection, data, originalDoc }) => {
          console.log('before change triggered', originalDoc, data);
          const lexicalField = collection.fields.find(
            (field) => 'name' in field && field.name === contentName,
          ) as LexicalEditorField;
          if (!lexicalField?.editor) {
            console.error('[LEXICAL MD HOOK]: No Editor found!');
            return data;
          }
          if (
            !lexicalField?.editor?.editorConfig?.resolvedFeatureMap?.has(
              'markdownConverter',
            )
          ) {
          ...
          if (
            (!originalDoc ||
              !isEqual(originalDoc[contentName], data?.[contentName])) &&
            data?.[contentName]
          ) {
            console.log(
              'content changed',
              JSON.stringify(data[contentName], null, 2),
            );
            ...
            const transformers =
              lexicalField.editor.editorConfig.features?.markdownTransformers ||
              TRANSFORMERS;
            try {
              headlessEditor.setEditorState(
                headlessEditor.parseEditorState(data[contentName]),
              ); // This should commit the editor state immediately
              const UPLOAD_TRANSFORMER = await BUILD_UPLOAD_TRANSFORMER(
                get(data[contentName], 'root.children'),
              );
            ...
            (!originalDoc ||
              originalDoc[markdownName] !== data?.[markdownName]) &&
            data?.[markdownName]
          ) {
